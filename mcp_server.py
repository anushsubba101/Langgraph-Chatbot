...
def list_expenses(start_date: str, end_date: str) -> str:
    ...
    if not rows:
        return "No expenses found for this date range."
    
    result = [{"date": r[0], "category": r[1], "amount": r[2], "description": r[3]} for r in rows]
    return str(result)

@mcp.tool()
def summarize(start_date: str, end_date: str) -> str:
